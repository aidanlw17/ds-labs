...
PRIV_TIMER_CHECK:
    			CMP R5, #MPCORE_PRIV_TIMER_IRQ // compare interrupt ID stored in R5 to private timer interrupt ID
                BNE KEYS_CHECK // If the ID does not match private timer, branch to check key ID

                BL PRIV_TIMER_ISR // ID matches, branch to interrupt service requesst for private timer
                B EXIT_IRQ

KEYS_CHECK:
    			CMP R5, #KEYS_IRQ // compare interrupt ID to keys interrupt ID

UNEXPECTED:     BNE UNEXPECTED // If ID does not match private timer or keys, it is unknown
                BL KEYS_ISR // ID matches, branch to ISR for keys
